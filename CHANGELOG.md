...
- BREAKING: `SymbolicAWEModels` compat is `0.18`, which renames `groups` to
...
- The wing carries the kite's 0.9 kg itself and its 16 points none, so its COM and
  inertia come from `ram_air_kite_body.obj`. Its body frame is set by reference
  points: y along the outer leading edges, z up to the inner stations, the origin
  between the inner leading edges. The YAML export and the `ram` and `4_attach_ram`
  factories give the same mass and frame; `simple_ram` gives the mass.
